...
<$ if ([6].includes(prj)) { $>
var secret = 'udemy-bafeisol' // 只是範例，實務上請使用更安全的字串
<$ } $>
/*
  Our settings - END
 */
...
function setCookieParser(app) {
  app.use(cookieParser(<$ if ([6].includes(prj)) { $>secret<$ } $>))
}
  <$ if ([6].includes(prj)) { $>
...
function getFeInitState(orders) { /* Fe = Front-end */
  var obj = {}
...
function queryOrder(req, res, next) {
  if (req.user) {
    var selectPart = 'SELECT `orders`.`product_id`, `orders`.`qty`, `products`.`price`'
    var fromPart = 'FROM `orders`, `products`'
    var wherePart = 'WHERE `orders`.`product_id` = `products`.`id` AND `orders`.`account` = ?'
    var sql = mysql.format(
      [selectPart, fromPart, wherePart].join(' '),
      [req.user.account],
    )
    con.query(sql, function (err, result, fields) {
      if (err) throw err
...
function renderMain(<$ if ([5, 6].includes(prj)) { $>req, <$ } $>res, result, page, theme, promotion, search) {
  res.render('main', {
...
function renderMember(res<$ if ([5, 6].includes(prj)) { $>, warning<$ } $>) {
  res.render('member', {
    page: 'member',
    search: '',
<$ if ([5, 6].includes(prj)) { $>
...
<$ if ([5, 6].includes(prj)) { $>
function createSession(<$ if ([6].includes(prj)) { $>req, <$ } $>res, account) {
  <$ if ([5].includes(prj)) { $>
  var sid = uuidv4()
  var sql = mysql.format(
...
function generateHash(password) {
  ...
  var passhash = crypto.createHash('sha256').update(password + salt).digest('hex')
...
function compareHash(password, hash) {
  ...
  return passhash === crypto.createHash('sha256').update(password + salt).digest('hex')
}
...
function setAppRoutes(app) {
  app.get('/',<$ if ([5, 6].includes(prj)) { $> queryOrder,<$ } $> function (req, res) {
    var sql = 'SELECT * FROM `products` ORDER BY `price`'
    con.query(sql, function (err, result, fields) {
      if (err) throw err
      renderMain(<$ if ([5, 6].includes(prj)) { $>req, <$ } $>res, result, 'index', '恣意享樂，盡在光速買！', true)
    })
  })

  app.get('/fashion',<$ if ([5, 6].includes(prj)) { $> queryOrder,<$ } $> function (req, res) {
    var sql = 'SELECT * FROM `products` WHERE `id` LIKE \'%fash%\' ORDER BY `price`'
    con.query(sql, function (err, result, fields) {
      if (err) throw err
      renderMain(<$ if ([5, 6].includes(prj)) { $>req, <$ } $>res, result, 'fashion', '率性穿搭，任你揮灑！')
    })
  })

  app.get('/food',<$ if ([5, 6].includes(prj)) { $> queryOrder,<$ } $> function (req, res) {
    var sql = 'SELECT * FROM `products` WHERE `id` LIKE \'%food%\' ORDER BY `price`'
    con.query(sql, function (err, result, fields) {
      if (err) throw err
      renderMain(<$ if ([5, 6].includes(prj)) { $>req, <$ } $>res, result, 'food', '大口咬下，就是爽快！')
    })
  })

  app.get('/search',<$ if ([5, 6].includes(prj)) { $> queryOrder,<$ } $> function (req, res) {
    ...
    con.query(sql, function (err, result, fields) {
      if (err) throw err
      renderMain(<$ if ([5, 6].includes(prj)) { $>req, <$ } $>res, result, 'search', '', false, search)
    })
  })
...
<$ if ([5, 6].includes(prj)) { $>
    ...
    con.query(sql, function (err, result, fields) {
      ...
      if (targetUser) {
        if (compareHash(password, targetUser.hash)) {
          createSession(<$ if ([6].includes(prj)) { $>req, <$ } $>res, account)
        } else {
          var warning = '密碼不正確'
  ...
  app.get('/cart',<$ if ([5, 6].includes(prj)) { $> queryOrder,<$ } $> function(req, res) {
    if (req.user) {
      var sql = mysql.format(
